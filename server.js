...
const User = mongoose.model('User', userSchema);
   ...
   const activitySchema = new mongoose.Schema({
    date: String,
	time: String, // This will now accept "14:23" instead of just "14:00"
    ...
    try {
        ...
        activityData.forEach(activity => {
            ...
            activity.interactions.forEach(interaction => {
				...
				const itemName = interaction.item ? interaction.item.trim() : 'N/A';
        ...
        // Add the recommendation text to the response object
        targetKeyDemographics.recommendationText = dynamicRecommendation;
        // ==================================
...
app.get('/api/activity/csv', authenticateToken, async (req, res) => {
    try {
        ...
        if (startDate || endDate) {
            query.date = {};
            if (startDate) query.date.$gte = startDate; // Greater than or equal to
            if (endDate) query.date.$lte = endDate;     // Less than or equal to
        }

        // Fetch data based on query (filtered or all)
        const activityData = await Activity.find(query).lean(); 
